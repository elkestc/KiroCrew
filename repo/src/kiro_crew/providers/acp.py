...
class AcpProvider(LLMProvider):
    ...
    _require_exact_binding: bool = False
    ...
    def __init__(
        ...
        self._require_exact_binding = require_exact_binding
        exact_binding = None
        if require_exact_binding:
            ...
            if not isinstance(model, str) or model.strip().lower() in (
                ...
                raise ValueError("Exact binding requires a concrete model identifier")
            if model != model.strip():
                raise ValueError("Exact binding requires an unambiguous model identifier")
            if (
                effort_per_model
                and model in effort_per_model
                and not is_valid_effort(effort_per_model[model])
            ):
                raise ValueError("Exact binding refuses an invalid reasoning effort")
            requested_effort = resolve_effort_for_model(
                model, slot_overrides=effort_per_model, defaults=effort_defaults
            )
            if not requested_effort:
                raise ValueError("Exact binding requires an explicit reasoning effort")
            exact_binding = (model, requested_effort)
        kwargs: dict[str, Any] = {
            "work_dir": work_dir,
            "model": model,
        ...
        if exact_binding is not None:
            kwargs["exact_binding"] = exact_binding
        if agent:
            kwargs["agent"] = agent
    ...
    async def _set_effort_config_option(self, level: str) -> None:
        ...
        """
        config_id = effort_config_id_for(self._client.backend)
        if self._require_exact_binding:
            if not self._client.supports_config_option(config_id):
                raise AcpBindingMismatch("Exact binding requires an advertised effort selector")
            await self._client.set_config_option(config_id, level)
            return
        if not self._client.supports_config_option(config_id):
            logger.debug("adapter exposes no %r config option; skipping effort push", config_id)
            return
        ...
        for candidate in ladder:
            try:
                ...
                if "unknown config option" in msg.lower():
                    ...
                    logger.debug("adapter rejected %r as unknown; skipping", config_id)
                    return
    ...
    async def change_effort(self, level: str) -> bool:
        ...
        """
        if self._require_exact_binding and level != self._resolve_effort():
            raise AcpBindingMismatch("Exact binding refuses a different reasoning effort")
        model = self._client._model
        ...
        config_id = effort_config_id_for(self._client.backend)
        if via_config_option and not self._client.supports_config_option(config_id):
            logger.info("change_effort skipped — adapter build exposes no %r option", config_id)
            return False
    ...
    async def clear_effort(self) -> bool:
        ...
        """
        if self._require_exact_binding:
            raise AcpBindingMismatch("Exact binding cannot clear its reasoning effort")
        model = self._client._model
        if not model_supports_effort(model):
            return False
        self._effort_per_model.pop(model, None)
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if self._require_exact_binding:
                self._client.check_exact_binding()
        except BaseException:
            if self._require_exact_binding:
