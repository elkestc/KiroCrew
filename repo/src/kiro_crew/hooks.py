...
def validate_file_path(raw: str, *, within_root: str | None = None) -> str | None:
    ...
    if within_root is not None:
        if ".." in Path(expanded).parts:
            return None
        # A caller's positive data boundary precedes realpath, stat and open.
        # Check the original spelling as well as the eventual resolved target.
        lexical = os.path.abspath(expanded)
        root = os.path.abspath(within_root)
        try:
            if os.path.commonpath((lexical, root)) != root:
                return None
        except ValueError:
            return None
        if os.name == "nt" and platform_compat.first_linked_ancestor(lexical) is not None:
            return None
    target = expanded
    if os.name == "nt":
    ...
    # Keep the original credential spelling. Resolving ~/.aws first can erase
    # its protected identity when that directory points at another filesystem.
    if is_sensitive_path(target):
        return None
...
def safe_read_file(path: str) -> str:
    ...
    if resolved is None:
        # No caller-controlled path reaches the log record through exc_info;
        # embedded newlines could otherwise forge a second record.
        raise PermissionError("Blocked: access to sensitive path or invalid path")
    try:
...
def safe_read_file_bytes_nolink(
    ...
    *,
    ...
    path = validate_file_path(raw, within_root=within_root)
    if path is None:
        return None
