...
def test_credential_dirs_are_refused() -> None:
    assert routes._is_sensitive_dir(fake_home_path() / ".ssh")
    # Plain credential dot-dirs the is_sensitive_path floor does not enumerate.
    assert routes._is_sensitive_dir(fake_home_path() / ".gnupg")
...
async def test_discover_local_protected_path(monkeypatch, tmp_path) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(routes, "_uploads_dir", lambda: tmp_path)
    resp = await routes._handle_discover(
        _Req({"kind": "local", "value": str(fake_home_path() / ".ssh")})  # type: ignore[arg-type]
    )
    assert resp.status == 200
...
def test_adopt_reused_copies_into_out_dir_and_omits_failures(tmp_path) -> None:
    ...
    copies = routes._adopt_reused(
        {
            "/": str(good),
            "/esc": str(escape),
            "/traversal": f"{probe_dir}/../dc-probe-a/esc.png",
    ...
    # Parent traversal is not an explicit allowed source spelling, even when
    # normalization would return to the same directory. Refusal preserves it.
    assert "/traversal" not in copies
    assert escape.read_bytes() == b"esc"
    for ref in ("/", "/esc"):
