...
.. list-table::
   ...
   * - ``ACP_BACKENDS_DEDICATED_TASK_SESSIONS``
     - pre-session registry query (task steps use the configured provider factory)
   * - ``ACP_BACKENDS_EXACT_BINDING``
     - pre-session registry query (which adapters can enforce exact model/effort readback)
...
# Task steps for these adapters use the normal provider/lease path. They do
# not expose the shared AcpRuntime that the Kiro task path adopts (H6).
ACP_BACKENDS_DEDICATED_TASK_SESSIONS = frozenset({ACP_BACKEND_CLAUDE, ACP_BACKEND_CODEX})

# These adapters expose model and effort readback in configOptions. Exact
# binding is an explicit operator constraint, independent of session sharing.
ACP_BACKENDS_EXACT_BINDING = frozenset({ACP_BACKEND_CLAUDE, ACP_BACKEND_CODEX})
...
# ``session/set_config_option`` with the id from ``effort_config_id_for``.
# A SEPARATE set from the model
...
def effort_config_id_for(backend: str) -> str:
    """Wire spelling of the effort selector; capability remains a separate check."""
    return "reasoning_effort" if backend == ACP_BACKEND_CODEX else "effort"
