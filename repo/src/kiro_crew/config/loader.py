...
class KiroCrewConfig:
    ...
    def _load_resolved(cls) -> tuple[KiroCrewConfig, int]:
        ...
        cfg = cls(
            agent=AgentConfig(
                ...
                require_exact_binding=_safe_bool(
                    agent_data.get("require_exact_binding", False), False
                ),
    ...
    def acp_effective_model(
        ...
        if global_model is None:
            ...
            if global_model == DEFAULT_MODEL and not self.agent.require_exact_binding:
        ...
        if not m:
            return ""
        if self.agent.require_exact_binding:
            # Admission must verify the caller's identifier, before registry
            # aliases can fold a model into a different version or family.
            return m
    ...
    def create_provider_factory(self) -> Callable:
        ...
        if self.agent.require_exact_binding and self.agent.fallback_model:
            raise ValueError("Exact binding requires agent.fallback_model to be empty")
        if self.agent.require_exact_binding and self.session.pool_size:
            raise ValueError("Exact binding requires session.pool_size=0")

        model = self.agent.model
        if model == DEFAULT_MODEL and not self.agent.require_exact_binding:
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            binding_kwargs: dict[str, Any] = (
                {"require_exact_binding": True} if self.agent.require_exact_binding else {}
            )
            return AcpProvider(
                work_dir=wdir,
                model=m,
