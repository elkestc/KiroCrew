...
from kiro_crew.agent_sdk.backends import ACP_BACKENDS_EXACT_BINDING, effort_config_id_for
...
class AcpBindingMismatch(AcpError):  # noqa: N818
    """An exact model/effort binding cannot be honored; no prompt may be sent."""
...
class AcpClient:
    ...
    _exact_binding: tuple[str, str] | None = None
    ...
    def __init__(
        ...
        exact_binding: tuple[str, str] | None = None,
    ):
        if exact_binding is not None:
            if acp_backend not in ACP_BACKENDS_EXACT_BINDING:
                raise ValueError("This adapter has no verified exact-binding readback")
            if (
                not isinstance(exact_binding, tuple)
                or len(exact_binding) != 2
                or not all(isinstance(value, str) and value.strip() for value in exact_binding)
            ):
                raise ValueError("Exact binding requires a model and reasoning effort")
    ...
    async def set_model(self, model_id: str) -> None:
        ...
        if self._exact_binding is not None and model_id != self._exact_binding[0]:
            raise AcpBindingMismatch(
                "Exact binding refuses a different model; create a new session"
            )
    ...
    async def _apply_startup_model(self) -> None:
        ...
        """
        if self._exact_binding is not None:
            # Use the original requested spelling, never a persisted/default
            # substitute. Missing/rejected selectors propagate to startup.
            try:
                await self.set_config_option("model", self._exact_binding[0])
            except AcpError as exc:
                raise AcpBindingMismatch(
                    "Exact model selection was refused by the adapter"
                ) from exc
            self._model = self._exact_binding[0]
            return
        if self._uses_advertised_model_selection:
    ...
    async def set_config_option(self, config_id: str, value: str) -> None:
        ...
        if self._exact_binding is not None:
            expected = dict(zip(("model", effort_config_id_for(self.backend)), self._exact_binding))
            if config_id in expected and value != expected[config_id]:
                raise AcpBindingMismatch("Exact binding refuses a different model or effort")
        if not self._session_id:
        ...
        # Adapters can return the full snapshot without a later notification.
        # Keep the reported values; an accepted request is not a local echo.
        config_options = response.get("configOptions") if isinstance(response, dict) else None
        if isinstance(config_options, list):
            self._acp_config_options = config_options
            self._sync_effort_levels()
        elif self._exact_binding is not None:
    ...
    def get_valid_effort_levels(self) -> list[str]:
        ...
        Uses the backend's wire selector id and extracts its options[].value
        list in the order ACP reported them.
        """
        config_id = effort_config_id_for(self.backend)
    ...
    def _reset_state(self) -> None:
        ...
        if self._exact_binding is not None:
            # A replacement transport must attest its own configuration. The
            # previous process's last snapshot cannot admit its first prompt.
            self._acp_config_options = []
            self._sync_effort_levels()
        ...
        try:
            ...
            for attempt in range(2):
                try:
                    ...
                    return
                except AcpBindingMismatch:
                    _startup_outcome = "exact_binding_refused"
                    await self._cleanup_failed_live_spawn()
                    self._reset_state()
                    raise
    ...
    async def _send_request(self, method: str, params: dict) -> int:
        if method == METHOD_PROMPT:
            self.check_exact_binding()
            if self._exact_binding is not None and params.get("sessionId") != self._session_id:
                raise AcpBindingMismatch("Exact binding cannot authorize another session")
    ...
    def check_exact_binding(self) -> None:
        """Require adapter readback immediately before a prompt reaches stdin.

        The binding survives transport resets; the adapter snapshot does not.
        Missing, duplicate or mismatched selectors therefore fail closed after
        reconnect as well as on initial startup. No requested-value echo counts
        as readback.
        """
        if self._exact_binding is None:
            return
        for config_id, value in zip(
            ("model", effort_config_id_for(self.backend)), self._exact_binding
        ):
            matches = [
                option
            ...
            if len(matches) != 1 or matches[0].get("currentValue") != value:
                raise AcpBindingMismatch(
                    "Exact model/effort admission refused: runtime readback does not match"
                )
