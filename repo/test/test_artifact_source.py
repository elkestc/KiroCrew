...
class TestNonGitProjectsLink:
    ...
    def test_marker_makes_a_project_root(self, narrow_tempdir, tmp_path: Path, marker: str) -> None:
        proj = tmp_path / "workplace" / "nrb" / "notes"
        proj.mkdir(parents=True)
        (
            (proj / marker).mkdir()
            if marker.startswith(".")
            else (proj / marker).write_text("x", encoding="utf-8")
        )
        target = _file(proj / "docs" / "spec.md")
        assert classify_source(target) == (LINK, str(proj))

    def test_nearest_marker_wins_in_a_monorepo(self, narrow_tempdir, tmp_path: Path) -> None:
    ...
    def test_unmarked_directory_still_copies(self, narrow_tempdir, tmp_path: Path) -> None:
        # No marker anywhere above it -> a snapshot, not a pointer.
        assert classify_source(_file(tmp_path / "loose" / "doc.md")) == (COPY, "")

    def test_verifier_agrees_with_the_classifier(self, narrow_tempdir, tmp_path: Path) -> None:
    ...
    def test_home_is_never_a_project_root(self, narrow_tempdir, fake_home: Path) -> None:
    ...
    def test_every_home_spelling_is_rejected(
        ...
        """
        original_home = str(fake_home_path())
    ...
    def test_a_real_project_under_home_still_links(self, narrow_tempdir, fake_home: Path) -> None:
