...
from test_isolation import collection_home_path
...
class TestChildEnv:
    ...
    def test_ssh_credentials_stripped(self, monkeypatch, tmp_path, name):
        ...
        monkeypatch.setenv(
            name, "/tmp/agent.sock" if name == "SSH_AUTH_SOCK" else "ssh -i /secret/key"
        )
        env = server._child_env(tmp_path)
...
class TestClientReadTimeout:
    ...
    def test_timeout_is_applied_to_the_connection(self):
        ...
        assert (
            socketserver.StreamRequestHandler.timeout is None
    ...
    def test_short_body_raises_incomplete(self):
        ...
        assert (
            probe.close_connection is True
...
class TestThreadGrowthIsBounded:
    ...
    def test_the_cap_is_enforced_on_both_append_paths(self):
        ...
        assert (
            src.count("MAX_THREAD_ENTRIES") >= 2
...
class TestQueueReadIsSizeBounded:
    ...
    def test_the_bound_is_checked_before_the_read(self):
        ...
        assert src.index("st_size") < src.index(
            "read_text"
...
class TestRemovingAProjectReleasesItsResources:
    ...
    def test_remove_stops_the_dev_process_and_the_static_listener(
        ...
        monkeypatch.setattr(server, "_stop_static_preview", lambda p: stopped.append(("static", p)))
...
class TestWhatIsWrittenStaysReadable:
    ...
    def test_anything_the_writer_accepts_the_reader_returns(self, isolated_queue):
        ...
        assert (
            fp.stat().st_size == server.MAX_RECORD_BYTES
        ), "the record no longer lands on the ceiling, so the boundary is untested"
        assert (
            server._read_request(fp) is not None
    ...
    def test_the_boundary_round_trip_stays_inside_an_io_budget(self, isolated_queue, monkeypatch):
        ...
        fp = server._request_file(server.QUEUE_DIR, _AT_LIMIT_RID)
        server._write_request(fp, _record_serialising_to(_AT_LIMIT_RID, server.MAX_RECORD_BYTES))
    ...
    def test_the_writer_bound_covers_every_route(self):
        ...
        assert (
            "max_bytes=MAX_RECORD_BYTES" in src
