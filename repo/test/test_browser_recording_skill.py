...
class TestRunnerValidation:
    ...
    def _run(self, *argv: str) -> subprocess.CompletedProcess:
        return subprocess.run([sys.executable, str(RUNNER), *argv], capture_output=True, text=True)
    ...
    def test_missing_playwright_fails_loud_without_install(
        ...
        p = self._run("--url", "http://127.0.0.1:1/", "--project", str(tmp_path))
        assert p.returncode != 0
...
class TestRecordedLineContainment:
    ...
    def test_forged_outside_path_rejected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with pytest.raises(SystemExit):
            self._main_with_driver_stdout(f"RECORDED {victim}\n", tmp_path / "out", monkeypatch)
    ...
    def test_last_marker_wins(self, tmp_path: Path, monkeypatch) -> None:
        ...
        self._main_with_driver_stdout(f"RECORDED {forged}\nRECORDED {real}\n", out, monkeypatch)
