...
class TestShortTmpBase:
    def test_posix_keeps_the_low_entropy_tmp(self, monkeypatch):
        ...
        monkeypatch.delenv("KIROCREW_TEST_ROOT")
    ...
    def test_explicit_test_root_precedes_platform_fallback(self, monkeypatch, windows):
        monkeypatch.setattr(platform_compat, "IS_WINDOWS", windows)
        assert short_tmp_base() == os.path.join(os.environ["KIROCREW_TEST_ROOT"], "tmp")
