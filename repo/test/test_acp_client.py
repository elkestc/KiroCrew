...
class TestInitializeSession:
    ...
    async def test_session_resume_success(self, tmp_path):
        ...
        session_dir = fake_home_path() / ".kiro" / "sessions" / "cli"
...
class TestModelEntitlementPreflight:
    ...
    @pytest.fixture(autouse=True)
    def _isolated_advertised_cache(self, monkeypatch):
        """These clients supply their own model list, with no prior wire cache.

        Another test on the same worker can advertise a different Claude wire
        spelling. Pin this fixture's initial state so its asserted request is
        independent of xdist scheduling and previously tested fake accounts.
        """
