...
class TestTokenPersistence:
    ...
    def test_update_only_changes_source_owned_fields(self, store):
        ...
        updated = store.update(entry["id"], label="CI callback", agent="oncall", enabled=False)
...
class TestFreshness:
    ...
    def test_resolve_context_tolerates_junk(self):
        ...
        assert webhooks.resolve_context({"summary": "legacy key", "registered_at": now}, now) == (
            webhooks.FRESHNESS_FRESH,
            "legacy key",
        )
...
class TestCrossOsPermissions:
    ...
    def test_the_temp_file_is_locked_down_before_any_payload_is_written(
        ...
        monkeypatch.setattr(webhooks.platform_compat, "restrict_to_owner", _measuring_restrict)
        ...
        assert sizes[0] == 0, (
            "the file already held payload bytes when it was locked down: " f"{sizes[0]} bytes"
        )
    ...
    def test_the_credential_directory_and_its_temp_files_are_all_gated(self):
        ...
        for target in targets:
            assert security.is_sensitive_path(str(target)), f"agent file tools must refuse {target}"
    ...
    def test_write_routes_permissions_through_windows_safe_helpers(self, store, monkeypatch):
