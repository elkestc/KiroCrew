...
class TestManagedServiceMarkerDetection:
    ...
    def test_launchd_definition_reads_environment_dictionary(self, monkeypatch, tmp_path):
        ...
        assert controller.installed_service_has_managed_marker() is False
        path.write_bytes(
            plistlib.dumps({"EnvironmentVariables": {"KIROCREW_SERVICE_MANAGED": "1"}})
        )
        assert controller.installed_service_has_managed_marker() is True
...
class TestLauncherReconcileIsProductionOnly:
    ...
    @pytest.fixture(autouse=True)
    def _real_home_predicate(self, monkeypatch):
        from kiro_crew import beacon, cli_server

        monkeypatch.setattr(cli_server, "is_default_home", beacon.is_default_home)
...
class TestAppArmorInstall:
    ...
    def test_exec_path_attaches_the_profile_to_the_resolved_launcher(
        ...
        os.chmod(
            launcher, 0o755
        )  # nosemgrep: python.lang.security.audit.insecure-file-permissions.insecure-file-permissions -- a launcher fixture must carry the exec bit a real venv launcher has; the file lives in a test-owned tmp dir.  # noqa: E501
        writes, runs, write, run = self._writers()
        monkeypatch.setattr(aa, "should_install", lambda: (True, "restricted"))
        monkeypatch.setattr(aa, "parser_path", lambda: "/usr/sbin/apparmor_parser")
        monkeypatch.setattr(aa, "parser_version", lambda _p: (5, 0))
...
class TestATakeoverOfTheAttachedPathIsRefused:
    ...
    def test_world_writable_directory_is_refused(self, tmp_path):
        """Reproduce a public temp directory using only disposable test data."""
        from kiro_crew.service import apparmor as aa

        public_tmp = tmp_path / "public-tmp"
        public_tmp.mkdir()
        public_tmp.chmod(0o777)
        problem = aa._substitutable_by_others(public_tmp)
...
class TestExpectedUidOverride:
    ...
    def test_a_foreign_owned_ancestor_is_refused(self, tmp_path, monkeypatch):
        ...
        os.chmod(
            foreign, 0o755
        )  # nosemgrep: python.lang.security.audit.insecure-file-permissions.insecure-file-permissions -- deliberately TIGHT against the 0o022 walk; the foreign OWNER below is what this test exercises.  # noqa: E501
        real_uid = app.stat().st_uid
