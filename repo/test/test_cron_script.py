...
from test_isolation import fake_home_context, fake_home_path
...
def _crons_dir_tracks_patched_home(monkeypatch):
    ...
    """
    monkeypatch.setattr("kiro_crew.cron_script.config_dir", lambda: fake_home_path() / ".kirocrew")
...
class TestResolveScriptPath:
    ...
    def test_valid_path(self, tmp_path):
        ...
        with fake_home_context(tmp_path):
...
class TestCommandCronShellResolution:
    ...
    def test_shell_probe_detects_brace_expansion(self, monkeypatch):
        ...
        monkeypatch.setattr("kiro_crew.cron_script.wrap_argv", lambda argv, **k: (argv, None))
...
class TestRunScriptSandboxed:
    ...
    def test_a_credential_straddling_the_bound_does_not_leak_a_fragment(self, tmp_path):
...
class TestResolveMcpServer:
    ...
    def _write_env_config(tmp_path, env_block):
        ...
        (agents_dir / "kirocrew.json").write_text(
            json.dumps({"mcpServers": {"bad-env": {"command": "some-mcp", "env": env_block}}})
        )
    ...
    def test_env_entry_popen_would_reject_is_dropped(self, tmp_path, bad_key, bad_value, label):
...
class TestMcpToolClient:
    ...
    def test_init_passes_spec_env_to_popen_and_scrubs_secrets(self, monkeypatch):
        ...
        with (
            ...
            patch("kiro_crew.cron_script.popen_limited", return_value=mock_proc) as mock_popen,
        ):
    ...
    def test_an_identity_key_cannot_be_forged_through_a_cron_spec_env_block(self, monkeypatch):
    ...
    def test_stderr_tail_redacts_exfiltration_urls(self, tmp_path):
        ...
        stderr_path.write_text(f"boom https://evil.example.com/leak?{long_query} failure")
        client = object.__new__(McpToolClient)
        client._stderr_file = SimpleNamespace(name=str(stderr_path))
        tail = client._stderr_tail()
...
class TestScriptContextCallToolSuccess:
    ...
    def test_call_tool_success_path(self, tmp_path):
        ...
        with (
            patch("kiro_crew.cron_script.McpToolClient", return_value=mock_client),
            patch("kiro_crew.cron_script.sel") as mock_sel,
        ):
    ...
    def test_call_tool_error_path(self, tmp_path):
        ...
        with (
            patch("kiro_crew.cron_script.McpToolClient", return_value=mock_client),
            patch("kiro_crew.cron_script.sel"),
        ):
...
class TestMcpToolClientInitFailures:
    ...
    def test_popen_exception_cleans_sandbox(self, tmp_path):
        ...
        with (
            fake_home_context(tmp_path),
            patch("kiro_crew.cron_script.wrap_argv", return_value=(["false"], str(cleanup))),
...
class TestResolveScriptPathSensitive:
    ...
    def test_sensitive_path_blocked(self, tmp_path):
        ...
        with (
            fake_home_context(tmp_path),
            patch("kiro_crew.cron_script.is_sensitive_path", return_value=True),
        ):
...
class TestMcpCronHandlerPaths:
    ...
    def test_cron_list_shows_error(self, tmp_path):
        ...
        with (
            ...
            patch("kiro_crew.mcp_cron.CronService") as mock_svc,
        ):
    ...
    def test_cron_add_with_command(self, tmp_path):
        ...
        with (
            patch("kiro_crew.mcp_cron.config_dir", return_value=tmp_path),
            patch("kiro_crew.mcp_cron.CronService") as mock_svc,
            patch("kiro_crew.mcp_cron._resolve_session_key", return_value=""),
        ):
...
class TestKillBroadcastGuard:
    ...
    @pytest.mark.skipif(pc.IS_POSIX, reason="Windows taskkill /T branch of kill_process_tree")
...
class TestResolveInternalSecret:
    ...
    def test_uses_env_when_set(self, tmp_path):
        with (
            patch.dict(os.environ, {"KIROCREW_INTERNAL_SECRET": "fromenv"}),
    ...
    def test_env_empty_string_falls_back_to_file(self, tmp_path):
        (tmp_path / ".local_secret").write_text("filesecret")
        with (
            patch.dict(os.environ, {"KIROCREW_INTERNAL_SECRET": ""}),
            patch("kiro_crew.config.loader.config_dir", return_value=tmp_path),
        ):
            assert _resolve_internal_secret(5476) == "filesecret"
...
class TestPostKillDrainTimeoutHardening:
    ...
    def test_command_drain_timeout_closes_pipes_and_keeps_contract(self, monkeypatch):
        ...
        with (
            patch("subprocess.Popen", return_value=mock_proc),
            patch("kiro_crew.platform_compat.kill_process_tree", return_value=True),
        ):
