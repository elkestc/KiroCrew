...
class TestGeneratedArtifactDefaults:
    ...
    def test_cdk_out_entry_is_load_bearing(self, store, pipeline, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            folder_watcher, "HARD_SKIP_DIRS", folder_watcher.HARD_SKIP_DIRS - {"cdk.out"}
        )
    ...
    def test_lock_files_stay_out_when_their_extension_is_readable(
        ...
        monkeypatch.setattr(
            FileReader, "SUPPORTED", FileReader.SUPPORTED | {".lock", ".lockb", ".lockfile"}
        )
        names = self._names(store, pipeline, self._lock_tree(tmp_path))
        assert "notes.md" in names
        assert names.isdisjoint(LOCK_FILES)
    ...
    def test_lock_globs_are_load_bearing(self, store, pipeline, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            tuple(g for g in folder_watcher.DEFAULT_IGNORE_GLOBS if g not in lock_globs),
        )
        names = self._names(store, pipeline, self._lock_tree(tmp_path))
...
class TestFolderWatcherScan:
    ...
    async def test_junk_files_create_no_state_rows(self, store, pipeline, tmp_path):
        ...
        rows = store.db.execute(
            "SELECT file_path FROM folder_file_state WHERE source_id = ?", (source_id,)
        ).fetchall()
...
class TestFolderWatcherPause:
    ...
    async def test_pause_stops_scan(self, store, pipeline, tmp_path):
        ...
        source_id = store.add_source(
            "test", "local_folder", str(vault), properties={"scan_paused": True}
        )
        source = {
            "id": source_id,
            "uri": str(vault),
    ...
    async def test_resume_after_pause(self, store, pipeline, tmp_path):
        ...
        fw = FolderWatcher(store, pipeline)
        source_id = store.add_source("test", "local_folder", str(vault), properties={})
        source = {
            "id": source_id,
            "uri": str(vault),
            "source_type": "local_folder",
...
class TestFolderWatcherFailedFiles:
    ...
    async def test_failed_ingestion_records_error(self, store, pipeline, vault):
        ...
        rows = store.db.execute(
            "SELECT status, error_message FROM folder_file_state WHERE source_id = ?", (source_id,)
        ).fetchall()
        for row in rows:
...
class TestAsyncToThread:
    ...
    async def test_last_seen_flush_and_commit_run_off_the_event_loop(
        ...
        monkeypatch.setattr(
            fw,
            "_load_state",
            lambda source_id: {str(note): {"status": "done", "mtime": 2.0, "item_ids": "[]"}},
        )
        ...
        await fw.scan_source({"id": "source", "uri": str(vault), "properties": "{}"})

        assert len(flush_threads) == len(commit_threads) == 1
        assert flush_threads[0] != loop_thread, "the last-seen flush ran on the event-loop thread"
    ...
    @pytest.mark.asyncio
    async def test_stale_claim_release_runs_off_the_event_loop(self, tmp_path, monkeypatch):
        vault = tmp_path / "vault"
        vault.mkdir()
        note = vault / "note.md"
        ...
        store.release_stale_claim.side_effect = lambda *args: release_threads.append(
            threading.get_ident()
        )
        pipeline = MagicMock()
        pipeline._dedup_enabled = False
...
class TestDeleteSourceCascade:
    ...
    def test_delete_folder_source_with_file_state(self, store):
        ...
        assert (
            store.db.execute("SELECT COUNT(*) FROM sources WHERE id = ?", (sid,)).fetchone()[0] == 0
        )
        assert (
            store.db.execute("SELECT COUNT(*) FROM items WHERE source_id = ?", (sid,)).fetchone()[0]
            == 0
        )
        assert (
            store.db.execute(
                "SELECT COUNT(*) FROM folder_file_state WHERE source_id = ?", (sid,)
            ).fetchone()[0]
            == 0
        )
...
class TestIngestTimeConfinement:
    ...
    async def test_a_path_that_escapes_the_root_is_refused_at_ingest_time(self, tmp_path):
        ...
        try:
            src = store.add_source(name="project", source_type="local_folder", uri=str(root))
            ...
            item_ids, outcome = await watcher._ingest_file(
                str(link), src, "default", {"confine_to_root": True}, [], root=str(root)
            )
...
class TestFailedIngestLeavesNoRetryLoop:
    ...
    async def test_failed_ingest_is_terminal_and_skipped_next_sweep(
        ...
        row = store.db.execute(
            "SELECT status FROM folder_file_state WHERE source_id = ?", (source_id,)
        ).fetchone()
        assert row["status"] == "failed"
    ...
    @pytest.mark.asyncio
    async def test_failed_row_keeps_the_version_that_failed(self, store, pipeline, tmp_path):
...
class TestInterruptedScanRetryCap:
    ...
    @pytest.mark.asyncio
    async def test_interrupted_row_is_retried_then_retired(self, store, pipeline, tmp_path):
        vault = tmp_path / "vault"
        vault.mkdir()
        ...
        assert (
            len(calls) == MAX_SCAN_ATTEMPTS
        ), f"expected at most {MAX_SCAN_ATTEMPTS} billed attempts, got {len(calls)}"
        row = store.db.execute(
            "SELECT status, attempts, error_message FROM folder_file_state " "WHERE source_id = ?",
            (source_id,),
        ).fetchone()
        assert row["status"] == "failed"
    ...
    @pytest.mark.asyncio
    async def test_row_already_stuck_scanning_converges(self, store, pipeline, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_successful_ingest_clears_the_retry_budget(self, store, pipeline, tmp_path):
        ...
        store.db.execute(
            ...
            (source_id, ok, "oldhash", 1.0, "[]", "2026-01-01", "scanning", MAX_SCAN_ATTEMPTS - 1),
        )
        store.db.commit()
        source = {
            "id": source_id,
    ...
    async def test_editing_a_capped_file_resets_its_budget_and_ingests(
        ...
        for _ in range(MAX_SCAN_ATTEMPTS):
            with contextlib.suppress(asyncio.CancelledError):
                await fw.scan_source(source)
        row = store.db.execute(
            "SELECT status, attempts FROM folder_file_state WHERE source_id = ?", (source_id,)
        ).fetchone()
    ...
    @pytest.mark.asyncio
    async def test_an_unchanged_capped_file_stays_retired(self, store, pipeline, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_a_retried_capped_file_gets_a_working_budget(self, store, pipeline, tmp_path):
        ...
        assert (row["status"], row["attempts"]) == (
            "scanning",
            1,
...
class TestFolderFileStateAttemptsMigration:
    def test_migration_adds_attempts_to_a_preexisting_db(self, tmp_path):
        ...
        try:
            cols = {
                r[1] for r in store.db.execute("PRAGMA table_info(folder_file_state)").fetchall()
            }
...
class TestDocumentStateTableSchema:
    ...
    def _columns(store, table: str) -> set[str]:
        return {
            r[1]
            for r in store.db.execute("SELECT * FROM pragma_table_info(?)", (table,)).fetchall()
        }
    ...
    def test_absent_state_tables_come_back_with_their_full_shape(self, tmp_path):
        ...
        try:
            for table in self._STATE_TABLES:
                assert (
                    self._columns(store, table) == expected[table]
