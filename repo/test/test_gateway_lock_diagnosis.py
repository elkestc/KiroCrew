...
def test_flock_is_held_by_a_fork_orphan(tmp_path, reap):
    ...
    if sys.platform == "linux":
        ...
        # Prove the real /proc parser sees this filesystem's live acquirer.
        # A PID namespace may later omit a lock whose acquiring PID is gone.
        with GatewayLock(home):
            assert platform_compat.flock_owner_pid(home / "gateway.lock") == os.getpid()
    ...
    if acquirer is None and os.environ.get("KIROCREW_TEST_ROOT") == "/test-root":
        recorded = int(lock_path.read_text().strip())
        assert not platform_compat.pid_exists(recorded)
        assert platform_compat.pids_holding_file(lock_path) == [orphan]
        assert platform_compat.parent_pid(orphan) == 1
        assert excinfo.value.holder_pid == recorded
        assert "holder could not be identified" in str(excinfo.value)
        assert "may be stale" in str(excinfo.value)
        assert "kill -9" not in str(excinfo.value)
        return
