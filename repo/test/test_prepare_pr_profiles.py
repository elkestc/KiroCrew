...
def test_charter_budgets_match_the_ci_workflows():
    ...
    assert f"≤{opus_blocking} BLOCKING, ≤{opus_advisory} advisory FINDING" in skill, (
    ...
    gpt_contract = (REPO_ROOT / ".github" / "review-prompts" / "gpt-review-core.md").read_text(
        encoding="utf-8"
    )
...
def test_every_floor_command_names_a_real_target():
    ...
    declared = json.loads((REPO_ROOT / "website" / "package.json").read_text(encoding="utf-8"))[
        "scripts"
    ]
...
def test_ci_blocking_scans_are_covered_by_the_floor():
    ...
    exempt_scripts = {
        # Execution boundary, not an independent scan. Its pytest/scanner
        # payloads are already covered by the floor and the script scan below.
        # Local execution must obey testing-conventions' offline boundary too.
    ...
    assert not npm_missing, f"ci.yml runs these npm scripts but the floor does not: {npm_missing}"
...
def test_scoped_runner_self_test_passes(tmp_path, monkeypatch):
    ...
    # The self-test builds argv; it never executes a frontend package manager.
    # Fail if that contract changes, without depending on an installed Node.
    for name in ("npm", "npx"):
        stub = tmp_path / name
...
def test_typecheck_script_actually_type_checks():
    ...
    """
    scripts = json.loads((REPO_ROOT / "website" / "package.json").read_text(encoding="utf-8"))[
        "scripts"
    ]
...
def test_gate_rationale_reference_exists_and_is_pointed_at():
    ...
    skill = (SKILL_DIR / "SKILL.md").read_text(encoding="utf-8")
    assert (
        "references/gate-floor.md" in skill
...
def test_readiness_context_flag_beats_env():
    ...
    ctx = pr_status.resolve_readiness_context(argv, {"PREPARE_PR_READINESS_CONTEXT": "Env Gate"})
