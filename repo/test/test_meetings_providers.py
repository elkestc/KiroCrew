...
class TestParseDt:
    ...
    def test_a_whole_day_date(self):
        assert cal._parse_dt("20260803", ";VALUE=DATE") == datetime(2026, 8, 3, tzinfo=timezone.utc)
...
class TestWindowsToIanaTable:
    ...
    def test_every_key_is_lowercase_and_stripped(self):
        ...
        """
        bad = [k for k in cal._WINDOWS_TO_IANA if k != k.lower() or k != k.strip()]
        assert bad == []
...
class TestParseIcs:
    ...
    def test_unescapes_text_values(self):
        text = _ics(_vevent(UID="esc", SUMMARY="A\\, B\\; C\\nD", DTSTART=_stamp(1)))
    ...
    def test_redacts_credentials_in_fields(self):
        text = _ics(_vevent(UID="c", SUMMARY="Review AKIAIOSFODNN7EXAMPLE", DTSTART=_stamp(1)))
...
class TestUrlValidation:
    ...
    def test_port_80_is_refused_even_though_the_shared_vet_allows_it(self, public_dns: None):
    ...
    def test_alternate_ip_encodings_are_refused_by_the_vet_not_the_resolver(self, host: str):
    ...
    def test_hosts_that_cannot_name_a_public_service_are_refused(self, host: str, public_dns: None):
...
class TestLocalIcsRead:
    ...
    def test_a_symlink_to_a_sensitive_target_is_refused(self, tmp_path: Path):
        ...
        """
        target = fake_home_path() / ".aws" / "credentials"
...
class TestRedirectLoopAgainstARealServer:
    ...
    @pytest.mark.asyncio
    async def test_a_redirect_with_no_location_is_refused(self, monkeypatch: pytest.MonkeyPatch):
        from aiohttp import web as aioweb
