...
def test_the_warning_omits_the_injection_note_when_none_fired() -> None:
    ...
    text = " ".join(_ingest_warnings([report], IngestConfig(granularity="turn", timeline="now")))
    assert "prompt-injection screen" not in text
    assert "dedup" in text


@pytest.mark.skipif(not (fake_home_path() / ".cache").exists(), reason="no cache dir on this host")
