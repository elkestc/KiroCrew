...
from test_isolation import fake_home_path, set_fake_home
...
class TestList:
    ...
    def test_touched_by_session_matches_the_real_mixed_key_formats(
        ...
        assert {x.slug for x in store.list(touched_by_session="dashboard:chat-1")} == {
            "read",
            "saved",
        }

    def test_touched_by_session_does_not_collapse_other_scopes(self, store: ArtifactStore) -> None:
    ...
    def test_touched_by_session_ignores_events_with_no_session(self, store: ArtifactStore) -> None:
...
class TestCreateKindInference:
    ...
    def test_create_infers_from_source_path(self, store: ArtifactStore, tmp_path: Path) -> None:
...
class TestBlankDocumentKind:
    ...
    def test_retyping_stays_available_between_structured_kinds(self, store: ArtifactStore) -> None:
        store.create(name="Untitled", content="", slug="u")
        store.update("u", content='{"a": 1}')
        assert store.update("u", content="<svg></svg>").kind == "svg"
    ...
    def test_snapshot_records_the_detected_kind_for_the_version(self, store: ArtifactStore) -> None:
        store.create(name="Untitled", content="", slug="u")
...
class TestSettleBlank:
    ...
    def test_a_settled_draft_respects_an_explicitly_chosen_kind(self, store: ArtifactStore) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(lambda s: s.set_pinned("u", True), lambda a: a.pinned, id="starred"),
        ],
    )
    ...
    def test_a_draft_is_rescued_even_when_deletion_is_forbidden(self, store: ArtifactStore) -> None:
    ...
    def test_deletion_is_refused_when_the_caller_forbids_it(self, store: ArtifactStore) -> None:
        self._blank(store)
        assert store.settle_blank("u", untitled_name=self.UNTITLED, allow_delete=False) == "kept"
        assert store.get("u") is not None
    ...
    def test_a_comment_is_seen_even_though_it_touches_no_field(self, store: ArtifactStore) -> None:
...
class TestSourceRootBarrier:
    ...
    def test_forged_root_cannot_widen_the_read_boundary(self, home_store, tmp_path) -> None:
    ...
    def test_relocate_promotes_a_copy_to_a_live_pointer(self, home_store, project_file) -> None:
    ...
    def test_rejected_writes_do_not_leak_descriptors(self, home_store, project_file) -> None:
        ...
        before = open_fds()
        for _ in range(40):
            assert home_store._try_write_source_path(str(outside), "nope", str(proj)) is False
    ...
    def test_write_preserves_extended_attributes(self, home_store, project_file) -> None:
    ...
    def test_write_preserves_the_original_file_mode(self, home_store, project_file) -> None:
    ...
    def test_refused_mirror_keeps_the_edit_and_demotes_to_copy(
        ...
        monkeypatch.setattr(type(home_store), "_try_write_source_path", lambda self, *a, **k: False)
    ...
    def test_relocate_clears_stale_source_root(self, home_store, project_file) -> None:
        ...
        home_dst = fake_home_path() / "moved.md"
...
class TestAllowedRootsSingleProducer:
    ...
    def test_set_contains_home_data_home_and_configured_roots(
        ...
        assert fake_home_path().resolve() in roots
        assert (fake_home_path() / ".kiro" / "crew").resolve() in roots  # data home
