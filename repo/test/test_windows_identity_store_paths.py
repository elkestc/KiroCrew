...
def _home_relative(path: Path) -> str:
    ...
    return path.relative_to(collection_home_path()).as_posix()
...
class TestReadersAgreeWithCanonicalTable:
    ...
    def test_usage_read_lists_are_the_projections(self) -> None:
        assert usage_api._CLI_SQLITE_DBS == ids.sqlite_dbs(Trust.TRUSTED, collection_home_path())
        assert usage_api._OTHER_SQLITE_DBS == ids.sqlite_dbs(Trust.OTHER, collection_home_path())
