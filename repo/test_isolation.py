...
class AllowedTestRoot:
    ...
    def validate(self, path: str | os.PathLike[str], _hops: int = 0) -> str:
        ...
        for index, part in enumerate(parts):
            ...
            if os.path.normcase(cursor) == os.path.normcase(self.root):
                root_seen = True
...
def _at_path(path, directory_fd=None):
    ...
    if directory_fd not in (None, -1) and not os.path.isabs(candidate):
        if _PLATFORM == "darwin":
            ...
            # Darwin F_GETPATH returns the path for this process's directory fd.
            parent = os.fsdecode(fcntl.fcntl(directory_fd, 50, bytes(1024)).split(b"\0", 1)[0])
        else:
            parent = os.readlink(f"/proc/self/fd/{directory_fd}")
...
def require_isolated_environment() -> AllowedTestRoot:
    ...
    global _BOUNDARY, _COLLECTION_HOME, _READONLY_RUNTIME
    ...
    if sys.platform == "linux":
        ...
        from native_test_boundary import verify_native_boundary

        root_path, _READONLY_RUNTIME = verify_native_boundary()
    root = AllowedTestRoot(root_path, allow_internal_links=True)
    root.validate(os.environ.get("HOME", ""))
    if not _inside(os.environ.get("HOME", ""), os.path.join(root.root, "homes")):
