...
# Before collection, import-time path bindings, capability probes, or fixtures.
# An environment variable alone is insufficient: the launcher must remove host
# mounts and networking. The verifier checks those properties before arming I/O.
_ALLOWED_TEST_ROOT = require_isolated_environment()


@pytest.fixture(autouse=True)
def _disposable_test_home(_isolation_dirs, _floor_monkeypatch):
    """Every test gets disposable home/config roots and fake credentials."""
    home = _isolation_dirs("fake-home")
    ...
    for name in tuple(os.environ):
        if name.startswith(
            ("AWS_", "AZURE_", "GOOGLE_", "GCP_", "CLOUDFLARE_", "R2_", "OPENAI_", "ANTHROPIC_")
        ):
            _floor_monkeypatch.delenv(name)
    for name in ("HOME", "USERPROFILE", "HOMEPATH"):
        _floor_monkeypatch.setenv(name, str(home))
    _floor_monkeypatch.setenv("HOMEDRIVE", "")
    for name, leaf in (
        ...
        _floor_monkeypatch.setenv(name, str(home / leaf))
    return home


@pytest.fixture
def fake_home(_disposable_test_home):
    """Public accessor; module-specific fake_home fixtures cannot replace the floor."""
    return _disposable_test_home
...
_SHORT_TMP_BASE = os.path.join(os.environ["KIROCREW_TEST_ROOT"], "tmp")
...
def _breadcrumb_guard(real):
    ...
    def _guarded(data_home):
        try:
            home_is_real = fake_home_path().resolve() == _REAL_USER_HOME.resolve()
...
_REAL_USER_HOME = fake_home_path()
...
@pytest.fixture(autouse=True)
def _isolate_agent_spec_home(
    _agent_spec_seam_modules, _isolation_dirs, _floor_monkeypatch, _disposable_test_home
):
    ...
    def _pinned_agents_dir() -> pathlib.Path:
        ...
        if (
            ...
            or paths.ambient_agents_dir() != _disposable_test_home / ".kiro" / "agents"
        ):
...
_REAL_KIRO_HOME_AT_START = (fake_home_path() / ".kiro").resolve()
...
@pytest.fixture(autouse=True)
def _isolate_kiro_sessions_dir(
    _isolation_dirs, _floor_monkeypatch, tmp_path_factory, _disposable_test_home
):
    ...
    if session_map is not None:
        monkeypatch.setattr(
            session_map,
            "_KIRO_SESSIONS_DIR",
            paths.kiro_sessions_dir() if paths is not None else root,
            raising=False,
        )
