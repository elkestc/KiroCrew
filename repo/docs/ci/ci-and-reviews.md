...
Linux pytest jobs use `scripts/ci_test_runner.py` after the local
`prepare-offline-tests` action installs dependencies. Tests run in the same
disposable mount and network namespaces used for local incident verification;
the checkout, runner home, Git authentication and cloud environment are absent.
The wrapper preserves shard selection and process failures, rejects absent or
non-passing reports, and exports only explicitly named regular coverage or
duration files. Coverage combines map `/test-root/repo/src/` back to `src/`.
Checker `--test` commands use the wrapper's command mode in that same boundary.
Fast Gate prepares only a standard-library runtime for those self-tests; static
diff checks retain their existing base-ref inputs outside the test process.

The Windows and macOS jobs still require native isolation integration. Running
their tests in a Linux container would not verify native platform behavior.
They remain merge blockers until compliant native execution is verified.
