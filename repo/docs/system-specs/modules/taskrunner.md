...
Task session allocation preserves the configured provider: adapters in
`ACP_BACKENDS_DEDICATED_TASK_SESSIONS` take the native dedicated-session path,
while runtime-capable task sessions retain their shared run runtime. The task
engine, dependency graph, review and checkpoint behavior are unchanged. See
[session](session.md) for allocation and lease ownership.
...
Replanning is admitted only while the run is `running`. A denied task gate,
pause, cancellation or terminal failure preserves its state and remaining
replan budget; it does not invoke a planner or append replacement tasks. This
applies to both sequential and parallel task groups.
...
- Without a Git diff, supplies the task description (up to 4,000 characters) and
  redacted execution result (up to 8,000 characters) to the independent reviewer.
  Text-only work is checked against its requested answer; file changes are
  required only when the task asks for them. The result is untrusted evidence,
  never review instructions.
