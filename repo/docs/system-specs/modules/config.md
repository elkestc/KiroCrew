...
`agent.require_exact_binding` defaults to false. When enabled, the native
provider factory requires `agent.fallback_model=""` and threads the constraint
into every provider it constructs. Each resolved model and reasoning effort
must form a concrete supported binding; role and per-session overrides are
validated after normal resolution, before client startup. The ACP client checks
the adapter's reported model and effort before sending any prompt. This option
does not select a provider or establish subscription authentication; see
[providers](providers.md) and [ACP client](acp-client.md).
