...
Effort discovery in the per-session client uses
`agent_sdk.backends.effort_config_id_for` for the selected backend: Codex's
`reasoning_effort` selector and Claude's `effort` selector have different wire
ids. Their advertised option order is preserved. The runtime-backed Kiro/KAS
session handle retains its existing `effort` parser.

`AcpClient.set_config_option` refreshes its public configuration snapshot from
the successful response's `configOptions`, including an empty list. Adapters
need not emit a separate update notification. The response supplies the actual
model, effort and mode selectors; the requested value is not echoed into the
cache. Missing or malformed option lists retain the last snapshot, and rejected
writes leave it unchanged. This update preserves the separately advertised
session modes and refreshes effort choices through the existing parser.

An optional immutable `exact_binding=(model, effort)` is supported only for
members of `agent_sdk.backends.ACP_BACKENDS_EXACT_BINDING`. Startup sends that
original model without substitution. `check_exact_binding` requires exactly one
matching model selector and one matching effort selector in the adapter's latest
reported configuration. `_send_request` checks this immediately before every
`session/prompt` write and refuses a different session ID. A transport reset
retains the requested binding while clearing the reported configuration, so a
reconnected process must provide fresh matching readback. Missing or malformed
configuration snapshots also invalidate strict readback:
this applies to initialization, successful setting responses and configuration
notifications. The legacy path still retains its last snapshot on missing data.
Changes to the bound model or effort are refused; a new binding requires a new session.
`AcpBindingMismatch` is a configuration refusal, not a transport retry at init.
This is configuration admission only; it does not verify subscription billing.
