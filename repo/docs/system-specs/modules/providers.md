...
The config-option effort channel uses the backend's wire id from
`agent_sdk.backends.effort_config_id_for`: `effort` for Claude and
`reasoning_effort` for Codex. Advertised-level discovery, the live capability
guard and both initial/live pushes use that same id. The channel's existing
best-effort startup and rejection step-down behavior remain; selecting the
correct wire id alone does not enforce an exact effort or subscription mode.
Offline adapter-frame regressions are in `test/test_codex_effort_transport.py`.

**Exact binding opt-in:** `require_exact_binding=True` requires a concrete model
and explicit valid effort on a backend in `ACP_BACKENDS_EXACT_BINDING`. It sends
the requested effort once, propagates a rejection, and refuses an absent
selector or mismatched configuration readback. It never descends the effort
ladder. A failed final startup check closes the owned client. The underlying
client also checks every outgoing prompt, including after reconnection; changing
or clearing a bound setting is refused. With the option off, the existing
best-effort behavior remains. `test/test_acp_exact_binding.py` covers admission,
wire refusal, reset, startup cleanup and native factory configuration. This
option is separate from authentication and subscription admission.
The native factory retains the original concrete model identifier before registry
translation, so readback is checked against the user's choice. An unresolved
`auto` is rejected unless the caller supplies a concrete override.
Exact mode also requires `session.pool_size=0`: warm claims currently reuse the
pool's binding without admitting the new claimant's original model and effort.
