...
Task steps for `ACP_BACKENDS_DEDICATED_TASK_SESSIONS` use that same
`get_or_create()` provider, model-resolution and semaphore path. They do not
bootstrap a shared runtime that their adapter does not expose. Kiro/KAS keep
their run-scoped shared-runtime task path. The opt-in is a pre-session registry
query; it grants no sharing, sandbox or approval exemption.
