...
When `agent.require_exact_binding` is enabled, provider factory construction
requires an empty `fallback_model`; a fallback chain cannot coexist with that
constraint. Each bound client also refuses a different model or effort and
checks readback at the prompt wire, so an alternate caller cannot silently
replace the seat's binding.
