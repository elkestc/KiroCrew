...
`ACP_BACKENDS_EXACT_BINDING` opts the Claude and Codex configuration-readback
adapters into an operator-requested fixed model/effort constraint. It grants no
sandbox, tool-routing or session-sharing exemption. Unsupported backends refuse
the explicit constraint before startup; the default provider path is unchanged.

`ACP_BACKENDS_DEDICATED_TASK_SESSIONS` selects the existing provider/lease path
for task steps whose adapters do not expose a shared run runtime. It is separate
from exact binding and session sharing. Kiro/KAS task runtime ownership stays
unchanged; the native task engine continues to own all execution state.
