...
For an operator who requires fixed settings at startup as well as explicit
switches, `agent.require_exact_binding=true` opts supported adapters into exact
model/effort admission. It requires an empty configured fallback and rejects
missing or mismatched runtime readback before a prompt. See
[providers](../modules/providers.md) for its scope and limitations.
The factory preserves the requested concrete provider identifier through admission,
including when a registry alias would select another version or model family.
An unresolved `auto` requires an explicit concrete override before construction.
Exact mode requires `session.pool_size=0` so each session admits its own binding.
