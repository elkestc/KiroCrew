...
## Mandatory disposable execution boundary

Host-connected pytest is forbidden. `scripts/run_isolated_tests.py` copies an
explicit source inventory to a new Linux temporary directory, then uses bubblewrap
with separate mount, PID, user and network namespaces, dropped capabilities and a
cleared environment. There are no host-home, WSL drive or service-socket mounts.
Writable test data resides under `/test-root`; the interpreter and system runtime
are separate read-only mounts. Dependency installation is a separate preparation
step, never an online fallback during tests.

Linux CI enters that same boundary through `scripts/ci_test_runner.py`. The
`prepare-offline-tests` composite action prepares a `/usr`-based Python virtual
environment and verifies user namespaces before tests start. Shard, coverage and
selection arguments are passed to pytest in the copied source tree. The wrapper
does not add marker exclusions; test selection belongs to the caller. The wrapper
requires a JUnit report with actual passing tests and preserves nonzero exits;
missing reports and empty or entirely skipped runs fail closed. Only explicitly
requested coverage and duration files are exported, after rejecting linked or
shared files on both sides. Raw test output remains in the disposable directory.
Source archiving screens the original root and every ancestor before invoking
Git or resolving a file, so a linked checkout cannot redirect preparation reads.

Root `conftest.py` calls `test_isolation.require_isolated_environment()` before
collection or capability probes. Its positive test-data boundary checks the
original lexical path before filesystem resolution, refuses escaping symlinks,
reparse points and subordinate mounts, and checks the fully resolved path.
Internal links are allowed only after their destinations pass the same root check;
the root itself and its ancestors must be unlinked. Runtime reads are restricted
to explicit read-only runtime roots and synthetic account configuration. This
Python audit hook is defense in depth; the OS namespace protects against native
extensions, subprocesses and check-to-use races. A denylist of familiar home
directories is not a test isolation boundary.

The standalone native evaluation prototype copies this source tree into a
Windows container with no mounts and `network=none`, or a default-deny macOS
sandbox. The macOS profile permits only public runtimes and disposable test data;
Unix sockets remain scoped to the test root. Both launchers clear the environment,
verify synthetic host canaries and network denial before collection, and report
only numeric outcomes, source locations and fingerprints. Native admission also
checks OS container/sandbox membership. Windows exercises real junctions; POSIX
descriptor and Linux memfd/mount tests retain capability-specific skips. This
prototype has not yet replaced the normal native CI jobs or completed their gate.

Every test gets a private `_disposable_test_home` fixture containing clearly
synthetic AWS credential/config files. A module's existing `fake_home` fixture
cannot replace this floor. Test code uses `fake_home` or `fake_home_path()` instead
of `Path.home()`; HOME, USERPROFILE and XDG directories are disposable. The runner
inherits no cloud variables, and the per-test fixture clears cloud-provider
variables left by preceding tests. Tests may set their own synthetic values.
The isolated collection home remains in place between tests and during teardown.

The credential-symlink incident, evidence handling, verification and outstanding
gate status are recorded in [the incident report](../../evaluation/security-incident.md).
...
### Config overrides
Tests that capture or resolve provider catalogs must isolate the in-memory
`model_registry._ADVERTISED_MODELS` cache as well as their disposable home. A
catalog from an earlier test can otherwise rewrite the wire model spelling in
an unrelated routing test on the same worker. The ACP coverage module uses a
fresh cache per test and restores the prior object on teardown.
...
Import-fallback tests must restore both the original `sys.modules` entry and
the parent package attribute. Restoring only an optional dependency can leave
two live copies of the importing module. The SQLite fallback tests preserve
those identities so a later MCP handler cannot catch a different driver's
exception class from the one its store raises.
