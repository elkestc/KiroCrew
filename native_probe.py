...
def payload(request_file):
    ...
    summary = {
        "linked_read_rejected": rejected[0],
        "linked_write_rejected": rejected[1],
    ...
    summary["passed"] = all(value for key, value in summary.items() if key != "network_errno")
...
def parse_payload(result):
    ...
    allowed = {
        "linked_read_rejected",
        "linked_write_rejected",
        "external_target_unchanged",
        "host_canary_read_blocked",
        "host_canary_write_blocked",
        "network_blocked",
        "network_errno",
        "passed",
    }
...
def windows_probe(source, owned, host_canary):
    ...
    request.write_text(
        json.dumps({"root": "C:\\test-root", "host_canary": str(host_canary)}), encoding="utf-8"
    )
    command = [
        "docker",
        "create",
        ...
        "C:\\probe",
        "--env",
        "HOME=C:\\test-root\\home",
        "--env",
        "USERPROFILE=C:\\test-root\\home",
        "--env",
        "TEMP=C:\\test-root\\tmp",
        "--env",
        "TMP=C:\\test-root\\tmp",
    ...
    try:
        ...
        bundle.mkdir()
        for name, path in [
            ("native_probe.py", source / "native_probe.py"),
        ...
        data.update(
            {
                "container_exit_code": state["State"]["ExitCode"],
        ...
        data["passed"] = (
            data["passed"] and result.returncode == 0 and state["State"]["ExitCode"] == 0
        )
        return data
    finally:
...
def macos_probe(source, owned, host_canary):
    ...
    request.write_text(
        json.dumps({"root": str(root), "host_canary": str(host_canary)}), encoding="utf-8"
    )
    ...
    if not result.stdout:
        raise RuntimeError(
            json.dumps({"exit_code": result.returncode, "stderr_sha256": digest(result.stderr)})
        )
    data = parse_payload(result.stdout)
    data.update(
        {
...
def main():
    ...
    (source / "probe-summary.json").write_text(
