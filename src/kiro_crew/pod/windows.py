...
def supervise_gateway(
    ...
    **The resource ceiling is applied here, and this is the only place it can
    ...
    try:
        ...
        # The window this marker covers opens HERE, not at the sighting: the
        # predecessor is already reaped, so `supervised_pid` fails closed and every
        # verb reads the pod as stopped until the successor is recorded. Teardown is
        # the only reader, and it refuses while this is fresh.
