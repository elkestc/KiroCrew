...
def main():
    ...
    for args in (
        ["init", "--template=", "--initial-branch=isolated-fixture", "."],
        ["add", "--force", "--all", "--", "."],
        ...
        if result.returncode:
            ...
            (root / "results/startup-summary.json").write_text(
                json.dumps(
                    {
                        "stage": "git:" + args[0],
    ...
    if sys.platform == "darwin":
        ...
        targets = set()
        for pattern in (
            "test_mcp_gateway_*.py",
            "test_socketsec.py",
            "test_platform_compat.py",
            "test_offline_boundary.py",
            "test_mcp_apps_e2e.py",
            "test_pod*.py",
            "test_service.py",
            ...
            for case in cases:
                for kind in ("failure", "error"):
                    ...
                    if failure is not None:
                        ...
                        tags = [tag for tag in ("PermissionError", "FileNotFoundError", "TestRootViolation",
                            "Operation not permitted", "AF_UNIX path too long", "ConnectionRefusedError",
                            "TimeoutError", "AssertionError", "AttributeError", "ValueError", "TypeError")
                            if tag in body]
                        locations = re.findall(
                            r"(?:test|src|scripts)[/\\][A-Za-z0-9_./\\-]+\.py:\d+", body
                        )
                        summary["failure_locations"].append(
                            {
                                "node_sha256": hashlib.sha256(
                                    (case.get("classname", "") + case.get("name", "")).encode()
                                ).hexdigest(),
                                "locations": sorted(set(locations)),
                                "tags": tags,
                                "errno": sorted(set(map(int, re.findall(r"\[Errno ([0-9]{1,5})\]", body)))),
